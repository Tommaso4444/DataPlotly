...
from qgis.PyQt.QtWidgets import (
    ...
    QVBoxLayout,
    QFileDialog,
    QMenu,QDialogButtonBox,QDialog,QDialogButtonBox,
    QLabel,QTableWidgetItem,QDialogButtonBox,QDialogButtonBox,
    QTableWidget
)
...
class DataPlotlyPanelWidget(QgsPanelWidget, WIDGET):  # pylint: disable=too-many-lines,too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, mode=MODE_CANVAS, parent=None, override_iface=None, message_bar: QgsMessageBar = None,  # pylint: disable=too-many-statements,too-many-arguments
        ...
        self.is_data_suitable_for_regression=False
        self.latest_scatter_plot=None
        self.latest_plot_factory=None
    ...
    def data_defined_color_updated(self):
        ...
        if self.ptype in ['scatter']:
            self.start_regression_analysis_button.setVisible(True)
        else:
            self.start_regression_analysis_button.setVisible(False)
    ...
    def create_plot(self,regression_mode=None):
        """
        ...
        x_field= self.x_axis_title.text()
                ...
                for _, v in self.plot_factories.items():
                    ...
                    if modified_code==True:
                        if v.trace[0].__class__.__name__=="Scatter":
                            ...
                            # print("self.x_data in create_plot");print(self.x_data)
                            # print("self.y_data in create_plot");print(self.y_data)
                            if self.is_data_suitable_for_regression==True:
                                self.start_regression_analysis_button.setVisible(True)
                            else:
                                self.start_regression_analysis_button.setVisible(False)
                                
                            
                            self.latest_plot_factory=v
                            self.latest_scatter_plot=v.trace[0]

                # print("self.x_data");print(self.x_data)

                self.plot_path = plot_factory.build_figures(self.ptype, pl, x_data=self.x_data, y_data=self.y_data, x_fields=[x_field], y_field=y_field,regression_mode=regression_mode)
    ...
    def updateScatterPlotWithRegressionLine(self,regression_mode):
        """
        updates only the LAST plot created
        get the key of the last plot created and delete it from the plot container
        and call the method to create the plot with the updated settings
        ...
        if self.mode == DataPlotlyPanelWidget.MODE_CANVAS:
            ...
            self.create_plot(regression_mode=regression_mode)
        else:
            self.widgetChanged.emit()
    ...
    def show_regression_results_dialog(self):
        ...
        self.updateScatterPlotWithRegressionLine(regression_mode)
        x_field= self.x_axis_title.text()
        y_field= self.y_axis_title.text()
        x_data=[[x] for x in self.x_data]
        x_fields=[];x_fields.append(x_field)
        
        
        self.regression_data = self.get_regression_data(x_data, self.y_data, x_fields, y_field)
