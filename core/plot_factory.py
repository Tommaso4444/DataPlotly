...
class PlotFactory(QObject):  # pylint:disable=too-many-instance-attributes
    ...
    def build_figures(self, plot_type, ptrace, x_data=None, y_data=None, config=None,x_fields=None, y_field=None,
                      regression_mode=None) -> str:
        """
        ...
        if plot_type in ('bar', 'histogram'):
            ...
            self.x_data=[[x] for x in x_data_single]
            self.y_data=scatter_trace['y']
            
            # print("self.x_data in plot_factory");print(self.x_data)
            # print("self.y_data in plot_factory");print(self.y_data)
            
            self.is_data_suitable_for_regression = self.x_data!=[] and self.y_data!=()
            # print("self.is_data_suitable_for_regression");print(self.is_data_suitable_for_regression)
            
            if self.is_data_suitable_for_regression==True:
                self.show_regression_results_dialog(x_fields,y_field)
            else:
                self.is_data_suitable_for_regression=False
            ...
            # ADD REGRESSION LINE
            ptrace=[]
            ptrace.append(scatter_trace);
            if regression_mode==True:
                regression_line_trace=go.Scatter(x=x_data_single, y=self.y_pred, mode='lines', line=dict(color='red', width=1.5), name="Regression Line")
                ptrace.append(regression_line_trace);
            figures = go.Figure(data=ptrace, layout=self.layout)
    ...
    def get_regression_data(self, x_data, y_data, x_fields, y_field):
        ...
        for i in range(len(x_fields)):
            # print("x_data in get_regression_data");print(x_data)
    ...
    def show_regression_results_dialog(self,x_fields,y_field):
        """Show a dialog with the regression summary results."""
  
        
        self.regression_data = self.get_regression_data(self.x_data, self.y_data, x_fields, y_field)
        ...
        self.y_pred = self.regression_data["y_pred"]
